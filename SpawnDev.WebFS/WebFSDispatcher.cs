...
namespace SpawnDev.WebFS
{
    ...
    public abstract class WebFSDispatcher : AsyncCallDispatcherSlim, IDisposable
    {
        ...
        public WebFSDispatcher(IServiceProvider serviceProvider, JsonSerializerOptions? jsonSerializerOptions = null, bool createNewScope = true)
        {
        ...
        /// <summary>
        /// This method should be called when the connection is closed
        /// </summary>
        ...
        /// </summary>
        protected virtual void SendReadyFlag() => SendCall(new object?[] { RemoteDispatcherMessageType.ReadyFlag, NeedRemoteReadyFlag, GetReadyFlagData() });
        ...
        public enum RemoteDispatcherMessageType
        {
            Unknown,
            Message,        // "."
            Call,           // "?"
        ...
        {
            {
                try
                {
                    var msgType = msg.Shift<RemoteDispatcherMessageType>();
                    switch (msgType)
                    {
                        case RemoteDispatcherMessageType.Message:
                            await HandleCall(msg, false);
                            return;
                        case RemoteDispatcherMessageType.Call:
                            await HandleCall(msg, true);
                            return;
                        case RemoteDispatcherMessageType.Response:
                            HandleReply(msg);
                            return;
                    ...
                    //if (DynamicCallables.TryGetValue(((int)msgType), out var dynamicCallable))
                    //{
                ...
                {
                    Console.WriteLine($"DataConnection_OnData: {ex.Message}");
                }
            }
        }
        /// <summary>
        ...
        {
            if (typeof(WebFSDispatcher).IsAssignableFrom(parameterType))
            {
                return this;
            }
        ...
        protected async Task<object?[]> PostDeserializeArgs(string msgId, MethodInfo methodInfo, Array? callArgs)
        {
            var methodParams = methodInfo.GetParameters();
            var ret = new object?[methodParams.Length];
            var callArgsLength = callArgs == null ? 0 : callArgs.Count;
            for (var i = 0; i < methodParams.Length; i++)
            {
        ...
        SendResponse:
            try
            {
                ...
                SendCall(new object?[] { RemoteDispatcherMessageType.Response, msgId, retError, retValue });
            }
            catch (Exception ex)
            {
                Console.WriteLine($"DataConnection.Send failed: {ex.Message}");
            }
            finally
            {
                StartRequestCleanup(msgId!);
        ...
        protected static object CreateTypedAction(Type[] paramTypes, Action<object?[]> arg)
        {
            ...
            {
                methodInfo = typeof(WebFSDispatcher).GetMethod(methodKey, BindingFlags.NonPublic | BindingFlags.Static);
